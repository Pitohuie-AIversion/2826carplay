...
2. 上传并部署云函数：
   ...
   - `getOpenid`
   - `vehicleCreate`
3. 在云函数控制台为 `bootstrapAdmin` 配置环境变量 `BOOTSTRAP_TOKEN`
   - 使用足够长的随机口令，不要把口令写入仓库或小程序代码
   - 未配置该变量时，初始化功能默认关闭
4. 首次配置时，使用开发版或体验版打开小程序“我的”页，点击“初始化管理员”，并输入上述口令
   - 仅当 `roles` 集合还没有任何记录时生效
   - 成功后会将当前账号写入 `roles` 集合并设为 `admin`
   - 正式版不显示初始化入口，口令只随本次初始化请求发送
5. 如需查看当前账号 openid，可点击“查询 OpenID”
   - 页面会自动复制当前用户 openid，并弹窗显示
6. 若 `roles` 已存在记录，则“初始化管理员”会返回锁定提示
   - 此时请由现有管理员在 `roles` 集合中手动新增或修改角色记录
   - 例如：`{ "openid": "复制出来的openid", "role": "admin" }`
7. 完成权限配置后，再进入“新增车辆”页面执行录入
8. 如需查看后台已录入车辆，可进入“我的”页并点击“车辆管理”
   - 支持按状态筛选：全部 / 在用 / 闲置 / 维修 / 停用
   - 支持按关键词搜索：车牌号、品牌型号
9. 如需修改车辆信息，可在“车辆管理”列表中点击“编辑车辆”
   - 当前支持修改：车牌号、车辆类型、品牌型号、注册日期、使用状态、公开可信摘要和仅后台可见的内部档案记录
   - 保存时会执行与新增相同的校验规则，并拦截重复车牌
10. 如需停用车辆，可在“车辆管理”列表中点击“停用车辆”
   - 当前采用软停用，不会物理删除数据库记录
   - 停用后车辆状态会更新为 `retired`
   - 已停用车辆可继续点击“恢复启用”
11. 如需恢复启用车辆，可在“车辆管理”列表中点击“恢复启用”
   - 当前会把车辆状态从 `retired` 恢复为 `idle`
   - 恢复后车辆会重新回到可管理状态
12. 如需彻底移除车辆，可在“车辆管理”列表中点击“删除车辆”
   - 当前为物理删除，车辆记录会从 `vehicles` 集合中移除
   - 存在任何预约历史（包括已取消或已完成）时禁止物理删除，请改用“停用车辆”
   - 删除前会弹出二次确认
   - 删除后不可恢复，建议先确认是否仅需“停用”
13. 如需查看单车完整信息，可在“车辆管理”列表中点击“查看详情”
   - 当前详情页展示：车牌号、品牌型号、车辆类型、注册日期、状态
   - 同时展示：公开可信档案状态、内部保养/检查/保险记录、VIN、发动机号、备注、创建人、创建时间、更新时间
   - 可从详情页继续跳转到“编辑车辆”
14. 如需维护车辆图片，可在“车辆详情”页使用图片管理区
   - 支持上传最多 9 张车辆图片
   - 支持从当前图片中选择一张设为封面
   - 支持移除已上传图片
...
16. 如图片删除因网络或云存储异常失败，可由管理员在“我的”页点击“存储清理”
   - 每次并行处理最多 5 条 `pending_file_deletions` 队列记录，避免默认超时下串行任务被提前终止
   - 删除成功后自动移除队列记录
   - 删除失败时保留记录，并更新重试次数和最近错误
   - 该入口仅管理员可见
...
- DUPLICATE_PLATE：车牌号已存在
  ...
  - 配置：为 `vehicles.plateNumber` 创建唯一索引，防止并发新增或修改产生重复车牌
