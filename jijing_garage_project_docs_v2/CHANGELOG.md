...
## 2026-08-22 Phase 16 云端部署与统计收口

完成阶段：Phase 16 开发、自动化测试和云端部署；真实微信身份验收延期统一执行。

修改文件：Phase 16 内容入口、分享归因、预约归因、分析、云函数配置、安全规则、测试、部署与阶段文档。

新增文件：`contentGuideList`、`contentGuideDetail`、`contentGuideManage` 云函数，`shared/contentAttribution.js`，Phase 16 测试与部署验收记录。

删除文件：无。

主要改动：
- 创建禁止客户端直连的 `content_guides` 集合及 3 个索引，部署 3 个新增和 4 个更新云函数。
- 发布一条关联真实在库车辆的取还车指南，云端公开列表、详情、字段隔离及无身份拒绝冒烟通过。
- 分享落地只在外部进入的页面栈首层计数，避免内容页继续进入车辆页后重复累计；内容预约开始只在预约页实际打开时计数。
- 报价确认写入匿名归因前再次校验内容 ID、车辆 ID、渠道和场景，非法历史归因不会进入 `analytics_events`；加固后的 `bookingQuoteRespond` 已于 14:03 重新部署并回读可用。

数据与隐私影响：匿名分析只保存事件类型、内容 ID、车辆 ID、渠道、场景和服务端时间；不保存 OpenID、预约 ID、姓名、手机号、备注或详细行程。

测试方式：`npm run check:release` 全部通过；26 个页面、56 个云函数，113 个测试套件 / 985 项测试，主包约 1.65 MiB。

已知问题：微信开发者工具登录已过期，管理员与普通用户的 Phase 13–16 真机预览、真实照片上传和完整履约链路尚未执行；该验收已按用户指令延期统一进行。

下一步建议：保持 Phase 17 决策门关闭；积累真实运营周期和履约样本后，再逐项决定是否评估支付、押金、证件或合同能力。
