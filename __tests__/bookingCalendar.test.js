...
describe("shared/bookingCalendar", () => {
  ...
  test("跨日期预约会占用范围内的每一天", () => {
    ...
    expect(view.cells.find((item) => item.date === "2026-07-11").ariaLabel).toContain("已选中")
    expect(view.cells.find((item) => item.date === "2026-07-11").ariaLabel).toContain("2 条预约")
    expect(view.cells.find((item) => item.date === "2026-07-11").ariaLabel).toContain("1 条待联系")
  ...
  test("只把同一车辆的重叠预约标为冲突", () => {
    ...
    expect(view.cells.find((item) => item.date === "2026-07-12").ariaLabel).toContain("存在同车预约重叠")
